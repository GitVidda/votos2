...
import Error from "./Error"; // Asegúrate de tener la ruta correcta hacia tu componente Error
import DatePicker from "react-datepicker";
import axios from "axios"

function CrearVotacion({ onVotacionCreada }) {
    ...
    // Función de navegación que permite desplazarse entre componentes
    const navigate = useNavigate();
    const today = new Date();

    // Función para agregar un nuevo candidato al crear una votación
    ...
    // Estado para la lista de candidatos que se monstrará por defecto (Para realizar una votación se necesita mínimo dos elementos)
    const [candidatos, setCandidatos] = useState([
        { nombre: "", apellido: "", nombreLista: "", numLista: "", },
        { nombre: "", apellido: "", nombreLista: "", numLista: "", }
    ]);

    // Función para quitar un candidato de la lista
    ...
    // Función que se ejecutará al guardar la votación creada
    const guardarVotacion = () => {
        ...
        if (nombreVotacion === "" || descripcionVotacion === "" || fechaInicio === "" || fechaFinalizacion === "" || candidatos.length < 2 || !candidatosSonValidos()
            ...
            // Realiza una solicitud POST para crear la votación en el servidor
            axios.post("http://localhost:8080/api/votacion", nuevaVotacion).then(response => {
                console.log("Nueva votación a ser creada:", nuevaVotacion);
    ...
    return (
        <div>
            ...
            <div className="flex justify-end items-center mr-7 mt-5 mb-5">
                <Link
                    to={`/Pantalla_Inicio/${iduser}`}
                    className="bg-indigo-600 hover:bg-indigo-800 text-white px-10 py-2 ml-4 rounded"
                >
                    Volver
                </Link>
            </div>
